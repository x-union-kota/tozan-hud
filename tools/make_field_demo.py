...
from gpx2route import (simplify, cumdist, total_gain, enc_poly, enc_ele, auto_ct, hav, _xy,
                       load_osm_ways, route_on_graph, bake_vec)
...
def snap_wps(pts, cum, wp_defs):
    ...
    for la, lo, n, t in wp_defs:
        ...
        wps.append({'d': round(best), 'n': n, 't': t})
    return wps
...
def build_osm(rid, name, fixture, corners, wp_defs, snap_r=200.0):
    """都市の周回: 角(lat,lon,ele)を OSM 道路網の歩道優先最短路で順に結ぶ。最後の角=最初の角"""
...
# ── 皇居ラン 1周(反時計回り・桜田門起点。内堀通り〜代官町通りの歩道) ──
KOKYO = build_osm('kokyo', '皇居ラン 1周', 'kokyo-osm.json',
